...
document.addEventListener('DOMContentLoaded', function() {
	$.each( nicks, function( key, value ) {
		$("<script></script>").appendTo('head').html('$(document).ready(function(){ \
			$("*").mouseover(function() { $(".p-channel_sidebar__name:contains(\'' + value[0] + '\'):not(:contains(\',\')), .p-channel_sidebar__name *:contains(\'' + value[0] + '\'):not(:contains(\',\'))").each(function(){$(this).text(\'' + value[1] + '\');});}); \
